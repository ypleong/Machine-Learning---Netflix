...
texts=[]
plt.scatter(horror_position[:,0], horror_position[:,1],s=40,c='r')
for label, x, y in zip(horror_names,horror_position[:,0],horror_position[:,1]):
#    plt.annotate("\n".join(wrap(label, 18)), xy=(x,y), xytext=(0,0), textcoords='offset points', bbox=dict(boxstyle='round,pad=0.5',fc='green',alpha=0.5),
 #                ha='right',va='bottom')#,arrowprops=dict(arrowstyle='->',connectionstyle='arc3,rad=0'))
    texts.append(plt.text(x,y,label, bbox={'pad':0, 'alpha':0}, size=11))
plt.scatter(children_position[:,0], children_position[:,1],s=40,c='b')
for label, x, y in zip(children_names,children_position[:,0],children_position[:,1]):
#    plt.annotate("\n".join(wrap(label, 18)), xy=(x,y), xytext=(0,0), textcoords='offset points', bbox=dict(boxstyle='round,pad=0.5',fc='yellow',alpha=0.5),
#                 ha='right',va='bottom')#,arrowprops=dict(arrowstyle='->',connectionstyle='arc3,rad=0'))
    texts.append(plt.text(x,y,label, bbox={'pad':0, 'alpha':0,'fc':'red'}, size=11))
plt.scatter(doc_position[:,0], doc_position[:,1],s=40,c='g')
for label, x, y in zip(doc_names,doc_position[:,0],doc_position[:,1]):
#    plt.annotate("\n".join(wrap(label, 18)), xy=(x,y), xytext=(0,0), textcoords='offset points', bbox=dict(boxstyle='round,pad=0.5',fc='blue',alpha=0.5),
#                 ha='right',va='bottom')#,arrowprops=dict(arrowstyle='->',connectionstyle='arc3,rad=0'))    
    texts.append(plt.text(x,y,label, bbox={'pad':0, 'alpha':0}, size=11))
xs = np.vstack((horror_position[:,0], children_position[:,0], doc_position[:,0])).tolist()
ys = np.vstack((horror_position[:,1], children_position[:,1], doc_position[:,1])).tolist()
adjust_text(texts, arrowprops=dict(arrowstyle="-", color='k', lw=0.1),
                        bbox={'pad':0, 'alpha':0.5}, size=11)
plt.show()
